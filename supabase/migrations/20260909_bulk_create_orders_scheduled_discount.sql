-- ==============================================================================================
-- 20260909_bulk_create_orders_scheduled_discount.sql
-- Description: áp giá của discount_programs (20260909_discount_programs.sql) vào giá bán server
-- tính lại — món nằm trong 1 chương trình đang bật + đúng lịch (giờ VN) thì dùng giá chương
-- trình thay vì products.price. Selling price luôn tính lại ở server (không tin client, xem
-- comment gốc ở 20260708_bulk_create_orders_server_pricing.sql) nên giảm giá tự động PHẢI
-- resolve ở đây, client chỉ mirror lại để hiện đúng giá trên nút món (xem
-- src/utils/discountPrograms.js).
--
-- Nhiều chương trình cùng khớp 1 món (hiếm, quản lý tự tạo chồng lịch) → lấy giá THẤP NHẤT cho
-- khách, không cộng dồn nhiều giảm giá.
--
-- Thân hàm giữ NGUYÊN VĂN từ 20260908_bulk_create_orders_takeaway_order_no.sql, chỉ chèn khối
-- resolve giá chương trình ngay sau khi đọc v_unit_price, trước khi cộng vào v_order_total.
--
...
DECLARE
  ...
  v_discount_price INTEGER; -- giá đã áp discount_programs cho món hiện tại, nếu có chương trình khớp
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Chương trình giảm giá đang bật + đúng lịch (giờ VN) cho món này → ghi đè giá gốc.
      -- days_of_week rỗng = không lọc theo thứ; start/end_date NULL = không giới hạn phía đó —
      -- 2 bộ lọc độc lập, dùng riêng hay kết hợp đều được.
      SELECT MIN(
        CASE dp.type
          WHEN 'fixed'  THEN dp.value
          WHEN 'amount' THEN GREATEST(v_unit_price - dp.value, 0)
          ELSE               ROUND(v_unit_price * (100 - LEAST(dp.value, 100)) / 100.0)
        END
      ) INTO v_discount_price
      FROM discount_programs dp
      JOIN discount_program_products dpp ON dpp.discount_program_id = dp.id
      WHERE dpp.product_id = v_product_id
        AND dp.address_id = v_address_id
        AND dp.enabled
        AND (dp.start_date IS NULL OR (now() AT TIME ZONE 'Asia/Ho_Chi_Minh')::date >= dp.start_date)
        AND (dp.end_date IS NULL OR (now() AT TIME ZONE 'Asia/Ho_Chi_Minh')::date <= dp.end_date)
        AND (
          dp.days_of_week = '{}'
          OR EXTRACT(DOW FROM now() AT TIME ZONE 'Asia/Ho_Chi_Minh')::SMALLINT = ANY(dp.days_of_week)
        );
