-- ============================================================
-- Admin Dashboard v7 — soft-delete addresses (xem
-- 20260824_addresses_soft_delete.sql) không được lẫn vào các số liệu/hoạt
-- động sống nữa, và việc xóa giờ tự lên "Hoạt động gần đây" để trace được.
--
-- Đổi so với v6:
--   - addresses_now / addresses_prev: loại deleted_at IS NOT NULL khỏi
--     total_addresses — không thì "tổng địa chỉ" phồng lên vĩnh viễn theo
--     số lần xóa.
--   - unactivated_addresses + mọi JOIN addresses trong attention_raw/
--     dormant_list (trừ 2 nhánh payment_review/payment_stale — vấn đề tiền
--     treo thì vẫn phải nhắc admin dù địa chỉ đã bị xóa): thêm
--     a.deleted_at IS NULL, địa chỉ đã xóa không còn "cần chú ý" nữa.
--   - activity_feed: thêm recent_deletions (type='deletion') — mỗi lần xóa
--     giờ hiện trong Hoạt động gần đây như thanh toán/đánh giá, thay vì biến
--     mất không dấu vết.
--
-- Không đổi signature → REVOKE/GRANT giữ nguyên, lặp lại cho nhất quán với
-- các migration trước (mọi CREATE OR REPLACE đều làm rơi search_path + cấp
-- lại EXECUTE cho PUBLIC, xem CLAUDE.md).
--
...
BEGIN
    ...
    addresses_now AS (
        SELECT COUNT(*) AS n FROM addresses WHERE deleted_at IS NULL
    ),
    addresses_prev AS (
        SELECT COUNT(*) AS n FROM addresses WHERE created_at < v_month_start AND deleted_at IS NULL
    ),
    dominant_prev AS (
    ...
    unactivated_addresses AS (
        ...
        WHERE a.deleted_at IS NULL
          AND NOT EXISTS (SELECT 1 FROM address_subscriptions s WHERE s.address_id = a.id)
    ),
    attention_raw AS (
        SELECT
        ...
        JOIN addresses a ON a.id = es.address_id AND a.deleted_at IS NULL
        ...
        FROM paid_signal ps
        JOIN addresses a ON a.id = ps.address_id AND a.deleted_at IS NULL
        LEFT JOIN users u ON u.id = a.manager_id
        ...
        JOIN addresses a ON a.id = te.address_id AND a.deleted_at IS NULL
        LEFT JOIN users u ON u.id = a.manager_id
        ...
        JOIN addresses a ON a.id = cr.address_id AND a.deleted_at IS NULL
        LEFT JOIN users u ON u.id = a.manager_id
        ...
        JOIN addresses a ON a.id = ti.address_id AND a.deleted_at IS NULL
        LEFT JOIN users u ON u.id = a.manager_id
    ...
    dormant_list AS (
        ...
        JOIN addresses a ON a.id = dp.address_id AND a.deleted_at IS NULL
        LEFT JOIN users u ON u.id = a.manager_id
    ...
    recent_deletions AS (
        SELECT 'deletion'::text, name, 'Đã xoá địa chỉ'::text, deleted_at
        FROM addresses
    ...
    activity_feed AS (
        ...
        UNION ALL SELECT * FROM recent_deletions
        ORDER BY 4 DESC
        LIMIT 20
    )
